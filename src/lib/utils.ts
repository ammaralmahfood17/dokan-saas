...
// Full flow: pending → confirmed → preparing → ready → completed
// NOTE: must match the CHECK constraint on orders.status in 001_init.sql.
export function getNextStatus(
  current: string
): 'confirmed' | 'preparing' | 'ready' | 'completed' | null {
  const flow: Record<string, 'confirmed' | 'preparing' | 'ready' | 'completed'> = {
    pending:   'confirmed',
    confirmed: 'preparing',
    preparing: 'ready',
    ready:     'completed',

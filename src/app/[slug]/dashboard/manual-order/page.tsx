              ...
              {cart.map(c => (
                <div key={c.id} className="flex items-center gap-2 bg-background rounded-lg p-2">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button onClick={() => setCart(prev => prev.filter(x => x.id !== c.id))} className="w-7 h-7 flex items-center justify-center text-destructive bg-card rounded touch-manipulation"><Trash2 size={12} /></button>
                  </div>
                </div>
              ))}
            </div>

              ...
              <div>
                ...
                {form.image_url && (
                  <button
                    ...
                    className="text-xs text-destructive hover:text-destructive block mt-1"
                  >
                    إزالة الصورة
                  </button>
                )}
          ...
          <div className="flex gap-4">
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <div
                ...
                className={cn(
                  'w-10 h-6 rounded-full transition-colors relative',
                  form.is_featured ? 'bg-warning' : 'bg-border'
                )}
              >
                <span className={cn(
                  'absolute top-1 w-4 h-4 bg-white rounded-full transition-transform',
              ...
              <div key={i} className="bg-background border border-border rounded-xl p-2.5 mb-2 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button type="button" onClick={() => setVariations(v => v.filter((_, j) => j !== i))}
                    className="w-11 h-11 flex-shrink-0 flex items-center justify-center text-destructive
                               active:text-destructive active:bg-destructive/10 rounded-lg touch-manipulation">×</button>
                </div>
              </div>
            ))}
          </div>
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-1.5">
                                ...
                                {item.is_featured && <Star size={13} className="text-warning flex-shrink-0" />}
                              </div>

...
export default function AnalyticsPage() {
  ...
  const statCards=[
    ...
    {icon:ShoppingBag,label:'طلبات اليوم',value:String(stats.ordersToday),color:'text-accent'},
    {icon:BarChart3,label:'إيرادات الأسبوع',value:formatBHD(stats.revenueWeek,'ar'),color:'text-accent'},
    {icon:Clock,label:'متوسط الطلب',value:formatBHD(stats.avgOrder,'ar'),color:'text-success'},
  ];
      ...
      {activeTab==='overview'?(
        <>
          ...
          {stats.ordersPending>0&&(
            <div className="card border-warning/30 bg-warning/10 flex items-center gap-3 py-3">
              <Clock size={16} className="text-warning"/>
              <span className="text-sm text-warning">{stats.ordersPending} طلب نشط</span>
            </div>
          )}

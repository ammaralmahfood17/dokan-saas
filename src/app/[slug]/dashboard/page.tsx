...
export default async function DashboardPage({
  ...
  const stats = [
    ...
    },
    {
      ...
      color: analytics.ordersPending > 0 ? 'text-warning' : 'text-muted-foreground',
    },
  ];
      ...
      {analytics.ordersPending > 0 && (
        <Link href={`/${slug}/dashboard/orders`}>
          <div className="flex items-center gap-3 bg-warning/15 border-warning/30
                          rounded-xl px-4 py-3 hover:border-warning/60 transition-colors">
            ...
            <div className="space-y-2">
              {recentOrders.map((order) => (
                <div key={order.id} className="card flex items-center gap-3 py-3">
                  <div className="ticket-number text-primary text-sm w-14 flex-shrink-0">
                    {order.order_number}
                  </div>
                  <div className="flex-1 min-w-0">

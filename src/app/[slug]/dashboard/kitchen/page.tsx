...
const CARD_STYLES: Record<string, string> = {
  ...
  confirmed: 'border-[#9FBBA9]',
  preparing: 'border-[#E4A05C] bg-[#FBF1E7]/60',
};
...
function KitchenCard({
  ...
  return (
    <div
      className={cn(
        'kitchen-card ticket-edge border-2 flex flex-col gap-3 relative pb-7',
        CARD_STYLES[order.status],
        isUrgent && 'border-destructive'
      )}
      style={{ '--ticket-backing': 'hsl(var(--sidebar))' } as CSSProperties}
    >
      {/* Elapsed warning */}
      {isUrgent && (
        <div className="absolute top-2 end-2 text-xs bg-destructive/15 text-destructive
                        px-2 py-0.5 rounded-full border border-destructive/40 ticket-number">
          {elapsedMin}m
        </div>
      )}
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="ticket-number text-2xl text-foreground">
          {order.order_number}
        </div>
      ...
      <div className="space-y-2 flex-1">
        {(order.order_items ?? []).map((item) => (
          <div key={item.id} className="flex gap-2">
            <span className="ticket-number text-primary text-sm w-6 flex-shrink-0">
              {item.quantity}×
            </span>
            <div>
              ...
              {item.notes && (
                <div className="text-xs text-warning mt-0.5">⚠ {item.notes}</div>
              )}
            </div>
          </div>
        ))}
      ...
      {/* Special notes */}
      {order.notes && (
        <div className="text-xs text-warning bg-warning/10 border border-warning/30
      ...
      {order.status === 'pending' ? (
        <div className="flex gap-2">
          <button onClick={handleAdvance} disabled={updating}
            className="flex-1 min-h-[44px] py-2.5 rounded-xl text-sm font-bold transition-all active:scale-[0.98] touch-manipulation bg-success hover:bg-success/90 text-success-foreground">
            {updating ? '...' : '✓ قبول الطلب'}
          </button>
          <button onClick={handleCancel} disabled={updating}
            className="flex-1 min-h-[44px] py-2.5 rounded-xl text-sm font-bold transition-all active:scale-[0.98] touch-manipulation bg-destructive/10 hover:bg-destructive/20 text-destructive border border-destructive/30">
            {updating ? '...' : '✕ إلغاء'}
          </button>
        </div>
      ) : order.status === 'confirmed' ? (
        <button onClick={handleAdvance} disabled={updating}
          className="w-full min-h-[44px] py-2.5 rounded-xl text-sm font-bold transition-all active:scale-[0.98] touch-manipulation bg-primary hover:bg-primary/90 text-primary-foreground">
          {updating ? '...' : '🍳 بدء التحضير'}
        </button>
      ) : order.status === 'preparing' && nextStatus ? (
        <button onClick={handleAdvance} disabled={updating}
          className="w-full min-h-[44px] py-2.5 rounded-xl text-sm font-bold transition-all active:scale-[0.98] touch-manipulation bg-success hover:bg-success/90 text-success-foreground">
          {updating ? '...' : '🔔 الطلب جاهز'}
        </button>
      ) : null}
    </div>

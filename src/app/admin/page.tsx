...
export default async function AdminOverviewPage() {
  ...
  const statCards = [
    ...
    { icon: CheckCircle,  label: 'اشتراكات نشطة',        value: String(stats.activeRestaurants), color: 'text-success' },
    { icon: Clock,        label: 'في التجربة',            value: String(stats.trialingCount),     color: 'text-warning' },
    { icon: TrendingUp,   label: 'إيرادات هذا الشهر',    value: formatBHD(stats.monthlyRevenue), color: 'text-primary' },
    { icon: AlertCircle,  label: 'متأخرة الدفع',          value: String(stats.pastDueCount),     color: stats.pastDueCount > 0 ? 'text-destructive' : 'text-muted-foreground' },
    { icon: CreditCard,   label: 'تنتهي قريباً (7 أيام)', value: String(stats.expiringSoon),     color: stats.expiringSoon > 0 ? 'text-warning' : 'text-muted-foreground' },
  ];
  ...
  return (
    <div className="p-4 sm:p-6 max-w-6xl space-y-6">
      ...
      {stats.pastDueCount > 0 && (
        <div className="flex items-center gap-3 bg-destructive/10 border border-destructive/40 rounded-xl px-4 py-3">
          <AlertCircle size={18} className="text-destructive flex-shrink-0" />
          <p className="text-sm text-destructive">
            {stats.pastDueCount} اشتراك متأخر في الدفع — يحتاج متابعة
          </p>
          <Link href="/admin/subscriptions?filter=past_due" className="ms-auto text-xs text-destructive hover:text-destructive">
            عرض ←
          </Link>
        </div>
      )}
      ...
      {stats.expiringSoon > 0 && (
        <div className="flex items-center gap-3 bg-warning/10 border border-warning/40 rounded-xl px-4 py-3">
          <Clock size={18} className="text-warning flex-shrink-0" />
          <p className="text-sm text-warning">
            {stats.expiringSoon} اشتراك ينتهي خلال 7 أيام
          </p>
          <Link href="/admin/subscriptions?filter=expiring" className="ms-auto text-xs text-warning hover:text-warning/80">
            عرض ←
          </Link>
        </div>
      )}
            ...
            <Link key={r.id} href={`/admin/restaurants/${r.id}`}>
              <div className="card-hover flex items-center gap-3 py-2.5">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <span className={`text-xs ${r.is_open ? 'text-success' : 'text-muted-foreground'}`}>

...
export async function middleware(request: NextRequest) {
  ...
  // ── Legacy locale-prefixed links (/ar/..., /en/...) ──
  // Dokan used to be bilingual with /ar and /en route prefixes. The app is
  // now Arabic-only and those prefixes no longer exist as real routes, but
  // old bookmarks, ads, and shared QR codes may still point to them. Strip
  // the prefix and redirect to the real (unprefixed) route instead of
  // letting these 404.
  const localePrefixMatch = pathname.match(/^\/(ar|en)(\/.*)?$/);

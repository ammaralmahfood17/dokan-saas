...
export function SubscriptionGate({ restaurantId, slug, children }: Props) {
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="card max-w-md text-center space-y-4">
          <div className="w-16 h-16 rounded-2xl bg-destructive/10 border border-destructive/30
            ...
            <CreditCard size={32} className="text-destructive" />
          </div>
  ...
  return (
    <>
      {showWarning && (
        <div className="bg-warning/10 border-b border-warning/40 px-4 py-2.5
          ...
          <Link href={`/${slug}/dashboard/settings`}
            className="ms-auto text-xs text-warning hover:text-warning whitespace-nowrap">
            تجديد →
          </Link>
        </div>
      )}
